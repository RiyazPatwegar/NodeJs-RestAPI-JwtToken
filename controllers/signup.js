            ...
            then((row, secondayData) => {
                ...
                if(row[0].length !== 1){
                    ...
                    bcrypt.compare(password, row[0][0].password).then((isEqual) => {
                        ...
                        if(isEqual){
                            const token = jwt.sign({email:email, id:row[0][0].id},'its-my-secret-key',{expiresIn:'1m'});
                            res.status(200).json({
                                statusCode: 200, 
                                message: 'Login successfull, Welcome user '+row[0][0].name,
                                token: token
                            });
                        }else{                            
                        ...
                        const error = new Error("Login failed, Password doesn't matched!");
                        error.statusCode = 400;
                        error.data = [{location: [{controller:'signup', method: 'login', data: err.array()}]}];
                        next(error);
                    });                                        
                }                
            }).            
